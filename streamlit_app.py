...
def get_openai_api_key():
    """環境変数、Streamlit Secrets、任意のローカル config.py から API キーを取得する。"""
...
def initialize_model():
    ...
    if not api_key:
        ...
        st.info("OPENAI_API_KEY を環境変数または Streamlit Secrets に設定してください。")
        return None
...
def generate_ai_response(prompt: str):
    ...
    try:
        ...
        if model is None:
            message = "⚠️ OpenAI APIキーが設定されていないため、回答を生成できません。OPENAI_API_KEY を環境変数または Streamlit Secrets に設定してください。"
            st.markdown(message)
