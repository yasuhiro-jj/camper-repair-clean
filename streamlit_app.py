...
def get_openai_api_key():
    """OpenAI APIキーを環境変数、Streamlit Secrets、任意のconfig.pyから取得"""
    ...
    try:
        ...
        return getattr(local_config, "OPENAI_API_KEY", "")
...
def generate_ai_response(prompt: str):
    ...
    try:
        ...
        if model is None:
            setup_message = "OpenAI APIキーが設定されていないため、回答を生成できません。OPENAI_API_KEYを環境変数またはStreamlit Secretsに設定してください。"
