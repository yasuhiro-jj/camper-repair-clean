...
def get_openai_api_key():
    """OpenAI APIキーをデプロイ環境で利用可能な場所から取得する。"""
...
def initialize_model():
    ...
    if not api_key:
        ...
        st.info("OPENAI_API_KEY 環境変数、Streamlit Secrets、またはローカルの config.py にAPIキーを設定してください。")
        return None
...
def generate_ai_response(prompt: str):
    ...
    try:
        ...
        if model is None:
            message = "OpenAI APIキーが設定されていないため、AI回答を生成できません。管理者に設定を確認してください。"
            st.warning(message)
